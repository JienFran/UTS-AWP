document.addEventListener("DOMContentLoaded", () => {
  ...
  const loadCampaigns = async () => {
    ...
    campaignContainer.innerHTML =
      '<p class="text-center text-gray-500 col-span-full">Memuat program donasi...</p>';
    try {
      ...
      if (campaigns.length === 0) {
        campaignContainer.innerHTML =
          '<p class="text-center text-gray-500 col-span-full">Belum ada program donasi.</p>';
      } else {
  ...
  const createCampaignCard = (campaign) => {
    ...
    return `
      <div class="border rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-shadow duration-300 flex flex-col bg-gray-50">
        ...
        }" alt="${campaign.title}" class="w-full h-52 object-cover">
        <div class="p-5 flex flex-col flex-grow">
          ...
          <div class="flex justify-between text-sm text-gray-700">
            <div><span class="font-semibold">Terkumpul</span><p class="font-bold">${formatRupiah(
              campaign.current_amount
            )}</p></div>
            <div class="text-right"><span class="font-semibold">Target</span><p>${formatRupiah(
              campaign.target_amount
            )}</p></div>
          </div>
        </div>
        <div class="p-5 border-t mt-auto bg-white"><a href="detail-kampanye.html?id=${
          campaign.id
        }" class="block text-center w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-lg transition">
  Donasi Sekarang
</a></div>
      </div>
    `;
